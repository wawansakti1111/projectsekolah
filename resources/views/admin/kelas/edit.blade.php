                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.kelas.update', $kelas->id) }}">
                        ...
                        @method('PUT') <div>
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $kelas->name)" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="user_id" :value="__('Wali Kelas')" />
                            <select name="user_id" id="user_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                <option value="">-- Pilih Wali Kelas --</option>
                                @foreach ($gurus as $guru)
                                    <option value="{{ $guru->id }}" @selected(old('user_id', $kelas->user_id) == $guru->id)>
                                        {{ $guru->name }}
                                    </option>
                                @endforeach
                            </select>
