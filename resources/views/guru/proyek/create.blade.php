        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('guru.proyek.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        {{-- ▼▼▼ TAMBAHKAN BLOK INI ▼▼▼ --}}
                        <div class="mt-4">
                            <x-input-label for="deadline" :value="__('Deadline Pengerjaan (Opsional)')" />
                        ...
                        <div class="mt-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <label for="sdg-{{$sdg->id}}" class="flex items-center space-x-3 p-3 border rounded-md hover:bg-gray-50">
                                    <input type="checkbox" name="sdgs[]" value="{{$sdg->id}}" id="sdg-{{$sdg->id}}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span>{{ $sdg->name }}</span>
                                </label>
                                @endforeach
                            </div>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('guru.proyek.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">
                                Batal
                            </a>
