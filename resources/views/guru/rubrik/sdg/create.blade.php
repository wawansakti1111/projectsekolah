<x-app-layout>
            ...
            <div class="bg-white/80 backdrop-blur-xl overflow-hidden shadow-2xl sm:rounded-3xl border border-green-200/50">
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('guru.rubrik.sdg.store') }}"
                          x-data="sdgRubricForm(@js($projects), @js(old('project_id')), @js(old('sdgRubrics')))"
                          x-init="init()"
                          class="space-y-6">
                        @csrf
    ...
    <script>
        function sdgRubricForm(projects, oldProjectId, oldSdgRubrics) {
            return {
                projects: projects,
                selectedProjectId: oldProjectId || '',
                availableSdgs: [],
                sdgRubrics: oldSdgRubrics && oldSdgRubrics.length > 0 ? oldSdgRubrics : [],
                ...
                removeSdgRubric(index) {
                    this.sdgRubrics.splice(index, 1);
                },
                addItem(sdgIndex) {
                    this.sdgRubrics[sdgIndex].items.push({ name: '', weight: '' });
                },
                removeItem(sdgIndex, itemIndex) {
                    this.sdgRubrics[sdgIndex].items.splice(itemIndex, 1);
                }
            }
        }
    </script>
