        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('guru.proyek.update', $proyek->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $proyek->title)" required autofocus />
                            </div>
                            <div>
                                ...
                                <select name="subject_id" id="subject_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->id }}" @selected(old('subject_id', $proyek->subject_id) == $subject->id)>{{ $subject->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>

                        {{-- Input untuk Deadline (Opsional) --}}
                        <div class="mt-4">
                        ...
                        <div class="mt-4">
                            <x-input-label for="attachment" :value="__('Ganti Materi (Opsional)')" />
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('guru.proyek.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">Batal</a>
                            <x-primary-button class="ms-4">Update Proyek</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
