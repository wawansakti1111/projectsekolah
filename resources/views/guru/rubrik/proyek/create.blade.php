<x-app-layout>
                ...
                <div class="flex items-center">
                    ...
                    <div>
                        <h2 class="font-bold text-3xl text-white leading-tight">
                            {{ __('Buat Rubrik Penilaian Proyek') }}
                        </h2>
                        <p class="text-green-100 text-sm mt-1">Buat rubrik penilaian untuk mengevaluasi proyek siswa</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="bg-white/80 backdrop-blur-xl overflow-hidden shadow-2xl sm:rounded-3xl border border-green-200/50">
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('guru.rubrik.proyek.store') }}"
                          x-data="rubricForm(@json(old('items')))" class="space-y-6">
                        @csrf

                        <div>
                            <h3 class="text-lg font-bold text-green-800 border-b border-green-200 pb-4">Informasi Rubrik Proyek</h3>
                            <div class="mt-6">
                                <!-- Dropdown Proyek Modern -->
                                <div x-data="{ open: false, selected: @js(old('project_id')), projects: @js($projects) }"
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <h3 class="text-lg font-bold text-green-800">Indikator Penilaian</h3>
                            <div class="text-sm font-bold">
                                Total Bobot: 
                                <span x-text="totalWeight" 
                                      :class="totalWeight !== 100 ? 'text-red-600' : 'text-green-600'">
                                </span>%
                            </div>
                        </div>
                        <p class="text-sm text-green-700/80 mb-4">Pastikan total bobot dari semua indikator adalah 100%.</p>

                        <!-- Alert Error Menarik -->
                        @if($errors->has('items') || session('toast_error'))
                            <div class="p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm font-medium flex items-start animate-fadeInUp">
                                ...
                                <span>{{ $errors->first('items') ?: session('toast_error') }}</span>
                            </div>
                        @endif
    ...
    @push('scripts')
    <script>
        // Animasi untuk alert (opsional, tapi bagus)
        ...
        window.initialRubricItems = @json(old('items'));

        function rubricForm() {
            return {
                items: [],
                init() {
                    let initialData = window.initialRubricItems;
                    if (Array.isArray(initialData) && initialData.length > 0) {
                        this.items = initialData;
                    } else {
                        this.items = [{ name: '', weight: '' }];
                    }
                },
                addItem() {
                ...
                get totalWeight() {
                    ...
                    return this.items.reduce((sum, item) => sum + Number(item.weight || 0), 0);
                }
            }
        }
    </script>
